...
export default function EditUsers() {
  ...
  const [tipo_user, setTipo] = useState('');
  ...
  useEffect(() => {
    async function getUser(){
      var response = await api.get('/api/users.details/'+id);
      
      setNome(response.data.name);
      setEmail(response.data.email);
      setTipo(response.data.tipo_user);
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          <Grid container spacing={3}>
            <Grid item sm={12} >
            <Button style={{marginBottom:18, marginRight:5}} variant='contained' href={'/admin/users'} >
              <ArrowBackIcon />
              Voltar
            </Button>
            <Button style={{marginBottom:18}} variant='contained' color='primary' href={'/admin/users/cadastrar'} >
              ...
              <Paper className={classes.paper}>
                ...
                <FormControl className={classes.formControl}>
                  <InputLabel id="labelTipo">Tipo de Usuário</InputLabel>
                  <Select
                    labelId="labelTipo"
                    id="tipo_user"
                    value={tipo_user}

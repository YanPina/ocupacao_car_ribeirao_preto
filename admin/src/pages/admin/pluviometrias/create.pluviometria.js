...
export default function CreatePluviometrias() {
  ...
  async function handleSubmit() {
    ...
    if(regiao!=='' && safra!==''){
      const response = await api.post('/api/pluviometrias/', data);

      if(response.status===200){
        window.location.href='/admin/pluviometrias'
      } else{
        alert('Erro ao cadastrar Pluviometrias!');
      }
    }else{

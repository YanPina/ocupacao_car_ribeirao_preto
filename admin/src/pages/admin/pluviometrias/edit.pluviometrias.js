...
export default function EditPluviometria() {
  ...
  useEffect(() => {
    async function getPluviometria(){
      var response = await api.get('/api/pluviometrias/'+id);
  ...
  async function handleSubmit() {
    ...
    if(regiao!=='' && safra!=='' && set!=='' && out!=='' && nov!=='' && dez!=='' && jan!=='' && fev!=='' && mar!=='' && abr!=='' && mai!=='' && jun!==''&& jul!==''&& ago!==''){
      const response = await api.put('/api/pluviometrias/'+id, data);

      if(response.status===200){
        window.location.href='/admin/pluviometrias'
      } else{
        alert('Erro ao atualizar o Pluviometria!');
      }
    }else{
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          <Grid container spacing={3}>
            <Grid item sm={12} >
            <Button style={{marginBottom:18, marginRight:5}} variant='contained' href={'/admin/pluviometrias'} >
              <ArrowBackIcon />
              Voltar
            </Button>

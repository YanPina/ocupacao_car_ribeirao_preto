...
export default function CreateBiomassa() {
  ...
  async function handleSubmit() {
    ...
    if(regiao!=='' && tch!==''){
      const response = await api.post('/api/produtividade/', data);

      if(response.status===200){
        window.location.href='/admin/produtividade'
      } else{
        alert('Erro ao cadastrar Produtividade!');
      }
    }else{
      alert('Preencha os campos "Região" e "TCH" .')
    }
  }
          ...
          <Grid container spacing={3}>
            ...
            <Button style={{marginBottom:18, marginRight:5}} variant='contained' href={'/admin/produtividade'} >
              ...
              <Paper className={classes.paper}>
                <h2>Cadastrar Produtividade Agrícola</h2>
